 ...
 */
static inline void __list_add(struct list_head *neu, struct list_head *prev, struct list_head *next)
{
    next->prev = neu;
    neu->next = next;
    neu->prev = prev;
...
static inline void hlist_move_list(struct hlist_head *old,
                   struct hlist_head *neu)
{
    neu->first = old->first;
    if (neu->first)
        neu->first->pprev = &neu->first;
    old->first = NULL;
}
